...
TIMESTEPS = 60
IMAGE = Image.open('Logos\AI-Series.jpeg')
...
if menu == 'Home':
    ...
    * **Data Source:** [Yahoo Finance] (https://finance.yahoo.com/)
    ...
    with col1:
        st.write("")
    with col2:
        stock_logo = Image.open(f'Logos\{selected_stock}.png')
    ...
    with col5:
        selected_layers = st.number_input("Layers:", 1, 3)
    with col6:
    ...
    if st.button('Predict'):
        ...
        local_model = load_model(MODEL_PATH + LAYER_FOLDER + f'\{selected_stock}_{selected_model}_{selected_layers}layers.h5')
        
        # Load the scaler
        scaler = SCALER_PATH + LAYER_FOLDER + f"\{selected_stock}_Scaler_{selected_layers}layers.pickle"
        ...
        with col7:
            st.write("")
        with col8:
            st.subheader(f"{selected_model} with {selected_layers} hidden layers")
        with col9:
            st.write("")
